...
public class Transcript {
    ...
    // This method checks if the student has taken all the courses that are required
    // for graduation
    ...
    public boolean checkEngineeringProjectAvailability() {
        ...
        for (int i = 0; i < takenCourses.size(); i++) {
            ...
            if (takenCourses.get(i).getCourseResult() == CourseResult.PASSED &&
                    Arrays.asList(restrictedCourseTypes)
                            .contains(takenCourses.get(i).getCourse().getCourseType()) == false
                    &&
                    Arrays.asList(restrictedCourses)
                            .contains(takenCourses.get(i).getCourse().getCourseCode()) == false) {
